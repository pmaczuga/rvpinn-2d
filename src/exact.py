...
class SinsExactSolution(ExactSolution):
    def __call__(x: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        sins = torch.sin(2*torch.pi*x) * torch.sin(2.0*torch.pi*t)
        res = sins
        return res

    def dx(x: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        sins = 2*torch.pi*torch.cos(2*torch.pi*x) * torch.sin(2.0*torch.pi*t)
        res = sins
        return res
    
    def dt(x: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
...
class ExpSinsExactSolution(ExactSolution):
    ...
    def dx2(x: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
    ...
    def dt2(x: torch.Tensor, t: torch.Tensor) -> torch.Tensor:
        exp1 = 4.0*torch.pi*torch.pi * torch.exp(torch.pi*(x-2*t)) *torch.sin(torch.pi * t)
        sin1 = torch.cos(2*torch.pi*x)
        res1 = exp1 * sin1
