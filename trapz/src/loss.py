...
def interior_loss(pinn: PINN, x:torch.Tensor, t: torch.tensor, epsilon: float):
    ...
    for n in range(1,20):
      ...
      for m in range(1,20):
        ...
        final_loss+=  \
          (0.25*(loss_tmp_weak1+loss_tmp_weak2+loss_tmp_weak3+loss_tmp_weak4)).sum().pow(2) * constant
...
#PRINTING
    ...
    x_loc1 = grids_loc[0].flatten().reshape(-1, 1).to(device)-0.5*h
    t_loc1 = grids_loc[1].flatten().reshape(-1, 1).to(device)-0.5*h
    x_loc2 = grids_loc[0].flatten().reshape(-1, 1).to(device)+0.5*h
    t_loc2 = grids_loc[1].flatten().reshape(-1, 1).to(device)-0.5*h
    x_loc3 = grids_loc[0].flatten().reshape(-1, 1).to(device)-0.5*h
    t_loc3 = grids_loc[1].flatten().reshape(-1, 1).to(device)+0.5*h
    x_loc4 = grids_loc[0].flatten().reshape(-1, 1).to(device)+0.5*h
    t_loc4 = grids_loc[1].flatten().reshape(-1, 1).to(device)+0.5*h
    ...
    dz_dx = 0.25*(dz_dx1+dz_dx2+dz_dx3+dz_dx4)
...
#    print(f'dz_dx_norm:{dz_dx_norm.item():.5f}')
    ...
    dz_dt = 0.25*(dz_dt1+dz_dt2+dz_dt3+dz_dt4)
#    #dfdt jest duze
#    for i in range(0,number2):
#      if dz_dt[i]>1:
#        print(dz_dt[i])
...
#    print(f'dz_dt_norm:{dz_dt_norm.item():.5f}')
    ...
    exact_dx = 0.25*(exact_dx1+exact_dx2+exact_dx3+exact_dx4)
...
#    print(f'exact_dx_norm:{exact_dx_norm:.5f}')
    ...
    exact_dt = 0.25*(exact_dt1+exact_dt2+exact_dt3+exact_dt4)
...
#    print(f'exact_dt_norm:{exact_dt_norm:.5f}')
    ...
    shift_dx_all = 0.25*(shift_dx_all1+shift_dx_all2+shift_dx_all3+shift_dx_all4)
...
#    print(f'shift_dx_norm:{shift_dx_norm:.5f}')
    ...
    shift_dt_all = 0.25*(shift_dt_all1+shift_dt_all2+shift_dt_all3+shift_dt_all4)
...
#    print(f'VM_norm:{VM_norm:.5f}')
    ...
    z = 0.25*(z1+z2+z3+z4)
    exact1 = exact_solution(x_loc1,t_loc1, epsilon) #the values are (0,1)
    exact2 = exact_solution(x_loc2,t_loc2, epsilon) #the values are (0,1)
    exact3 = exact_solution(x_loc3,t_loc3, epsilon) #the values are (0,1)
    exact4 = exact_solution(x_loc4,t_loc4, epsilon) #the values are (0,1)
    exact = 0.25*(exact1+exact2+exact3+exact4)
